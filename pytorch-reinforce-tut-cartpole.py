"""
Reinforce code stolent from https://github.com/pytorch/examples/blob/master/reinforcement_learning/reinforce.py
"""
...
class SurprisalModel(nn.Module):
    def __init__(self):
        super(SurprisalModel, self).__init__()
...
def pretrain_surprise_model():
    ...
    for _ in range(1):
        ...
        print('First MSE: {}. Last: {}'.format(stored_mses[0], stored_mses[1]))
...
def main():
    ...
    for i_episode in range(500):
        ...
        for t in range(1000):  # Don't infinite loop while learning
            ...
            prev_state = state
            action = select_action(state)
            state, real_reward, done, _ = env.step(action)
            # if args.render:
            env.render()
            ...
            reward_norm = reward / (np.std(stored_mses))
